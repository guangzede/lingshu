...
const FiveElementsAnalysis: React.FC<Props> = ({ metal=100, wood=70, water=20, fire=60, earth=50 }) => {
  const items = React.useMemo(
    () => [
      { key: '金', value: metal, color: '#f6c453' },
      { key: '木', value: wood, color: '#7aa28c' },
      { key: '水', value: water, color: '#5bbcff' },
      { key: '火', value: fire, color: '#ff7b7b' },
      { key: '土', value: earth, color: '#d8c37a' }
    ],
  ...
  React.useEffect(() => {
    ...
    query.select('#five-elements-radar').node().exec((res) => {
  ...
  return (
    <View className="five-elements-card glass-card">
      <View className="card-header">
        ...
        <Text className="card-section-guide">金木水火土能量雷达图</Text>
      </View>
      <View className="card-body">
        <View className="radar-wrap">
          <Canvas className="radar-canvas" id="five-elements-radar" canvasId="five-elements-radar" type="2d" />
