...
import type { DaliurenCaseListItem } from '@/types/daliurenCase'
import './index.scss'

const HistoryPanel: React.FC = () => {
  const { getSavedCases, loadCase, deleteCase, setActiveTab } = useDaliurenStore((s) => s)
  const [cases, setCases] = React.useState<DaliurenCaseListItem[]>([])
  ...
  return (
    <View className='daliuren-history-panel'>
              ...
              <View key={item.id} className='case-card'>
                <View className='case-header'>
                  <Text className='case-number-title'>
                    {idx + 1}. {item.name || item.subject || '未命名排盘'}
                  </Text>
      ...
      <View className='history-note'>
        <Text>提示：记录仅保存在当前账号，支持搜索姓名/主题/日期，删除后不可恢复。</Text>
      </View>
    </View>
  )
}
